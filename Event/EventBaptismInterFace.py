...
class EventBaptism_MessageBox(MessageBoxBase):
    ...
    def __init__(self, parent=None):
        ...
        # self.BaseMessageBoxWidget.label_11.setText("性别")
    ...
    def get_InputEvenBaptismMessageinfo(self):
        evenBaptism_messageinfo = {
            'holyevent_p1_id': '',
            'holyevent_p1_name': self.BaseMessageBoxWidget.inputLine_1.text(),
            'holyevent_p1_holyname': self.BaseMessageBoxWidget.inputLine_2.text(),
            'holyevent_p1_gender': self.BaseMessageBoxWidget.inputLine_9.currentData(),
            'holyevent_p2_id': '',
            'holyevent_p2_name': '',
    ...
    def set_InputEventBaptismMessageinfo(self, evenBaptism_messageinfo):
...
class EventBaptism_Main_Interface(QWidget):
    def __init__(self, cur_parish_id=1):
        ...
        self.EventBaptism_all_info = None
        ...
        self.BaseMainInterface.label_2.setText("添加事件")
        main_layout.addWidget(self.BaseMainInterface)  # 正确地将 ReusableWidget 作为一个整体添加到布局中

        self.resize(800, 600)

        self.BaseMainInterface.BaseQuery.addButton.clicked.connect(self.add_evenBaptism)
        self.BaseMainInterface.BaseQuery.delButton.clicked.connect(self.delete_evenBaptism)
        self.BaseMainInterface.BaseQuery.ModButton.clicked.connect(self.modify_evenBaptism)
        ...
        self.Load_evenBaptism(QUERY_TYPE.QUERY_ALL, self.evenType, self.cur_parish_id)

    def query_evenBaptism_info_with_like(self):
        if self.BaseMainInterface.BaseQuery.searchInput.text() == "":
            self.Load_evenBaptism(QUERY_TYPE.QUERY_ALL, self.evenType, self.cur_parish_id)
        else:
            self.Load_evenBaptism(QUERY_TYPE.QUERY_LIKE, self.evenType, self.BaseMainInterface.BaseQuery.searchInput.text())

    def Load_evenBaptism(self, query_type, even_type, query_param):
        with HolyEventDB() as db:
            if query_type == QUERY_TYPE.QUERY_ONE:
                self.EventBaptism_all_info = db.fetch_all_event_by_type(even_type)
            elif query_type == QUERY_TYPE.QUERY_ALL:
                self.EventBaptism_all_info = db.fetch_even_with_like(even_type, query_param)
            elif query_type == QUERY_TYPE.QUERY_LIKE:
                self.EventBaptism_all_info = db.fetch_all_event_by_type(even_type)
            else:
                return

        if self.EventBaptism_all_info is None:
            return
        ...
        self.BaseMainInterface.BaseQuery.set_viewWidget_data(header_info, self.EventBaptism_all_info)

    def add_evenBaptism(self):
        ...
        if w.exec():
            with HolyEventDB() as db:
                ...
                db.add_even_with_single(get_InputEvenBaptismMessageinfo)
            self.Load_evenBaptism(QUERY_TYPE.QUERY_ALL, self.evenType, self.cur_parish_id)

    def delete_evenBaptism(self):
        idx = self.BaseMainInterface.BaseQuery.tableWidget.currentRow()
        if idx != -1:
            w = EventBaptism_MessageBox(self)
            w.titleLabel.setText("删除事件")
            w.set_lineedit_uneditable()
            w.set_InputEventBaptismMessageinfo(self.EventBaptism_all_info[idx])
            if w.exec():
                with HolyEventDB as db:
                    db.delete_event(self.EventBaptism_all_info[idx]["holyevent_id"])
                    self.Load_evenBaptism(QUERY_TYPE.QUERY_ALL, self.evenType, None)

    def modify_evenBaptism(self):
        idx = self.BaseMainInterface.BaseQuery.tableWidget.currentRow()
        if idx != -1:
            w = EventBaptism_MessageBox(self)
            ...
            if w.exec():
                with HolyEventDB() as db:
                    EvenBaptism_info = w.get_InputEvenBaptismMessageinfo()
                    EvenBaptism_info["student_id"] = self.EventBaptism_all_info[idx]["student_id"]
                    EvenBaptism_info["student_school_id"] = self.cur_parish_id
                    db.update_student(EvenBaptism_info)
                self.Load_evenBaptism(QUERY_TYPE.QUERY_ALL, self.evenType, self.cur_parish_id)
